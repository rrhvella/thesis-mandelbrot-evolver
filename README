...
Hovering on a thumbnail in the ‘fractal selection panel’.	|	Replaces the contents of ‘genome info’ with the textual representation of the genome represented by that thumbnail. Pressing the ‘Enter’ key.	|	* Iterates the genetic algorithm and replaces the thumbnails in the ‘fractal selection panel’ with the ones for the new generation.<br>* Increases the number in the ‘generations label’ by 1.<br>* Resets the scores of all the genomes.
Clicking the output button.	|	Saves the genome displayed in the ‘fractal image preview’ to disk. See text below for more info.

As this project was developed based on ideas discussed in published works, a list of references can be found in the file, 'REFERENCES'.
